...
$("#delBtn").click(function () {
    if (getChk() == "") {
        messageBox.notify("请选择要删除的stock!", "danger");
        return;
    }

    messageBox.confirm("是否删除选中的stock？", function (result) {
        messageBox.showLoading('正在处理请稍后...');
        $.ajax({
            type: "POST",
...
$("#hisSylBtn").click(function () {
    ...
    if (getChk() == "") {
        messageBox.notify("请选择stock!", "danger");
        return;
    } else if (ids.length > 5) {
        messageBox.notify("最多选择5支stock对比!", "danger");
        return;
    }
...
$("#hisFhBtn").click(function () {
    if (rowindex == null) {
        messageBox.notify("请选择需要查看的stock!", "danger");
        return;
    }
